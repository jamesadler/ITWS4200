...
// Adds any hashtags from the tweet to a hashtag list.
function tweetTicker(data,curTweet){
	setTimeout(function(){
		$('#twitterFeed li:first').animate( {marginTop: '50px'}, 1000, function(){
			...
			var hashtags = data[curTweet].entities.hashtags;
			if(hashtags.length>0){
				for(var i=0; i<hashtags.length; i++){
					$("#hashtagFeed").append("<li class='hashtag' id='hashtag"+curTweet+"'>#"+hashtags[i].text+"</li>");	
				}
			}
