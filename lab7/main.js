...
angApp.controller("TwitterCtrl", function($scope, $http){
	...
	$scope.createDB = function() {
		$.post("/createDB", function(data){});
	}
	
	$scope.getTweets = function(option) {
		$("#twitterFeed").html("");

		// Get tweet data from the json file
		if(option === 1){
		...
		else if(option === 2){ // get tweet data from the mongo db
	...
	$scope.convertMongo = function() {
		...
		alert("Converting mongo database to JSON ("+fileName+")");
		...
		$.post("/getTweets", function(data){
			...
			// Sets up the download link
			$("#downloadLink").html('Download '+fileName);
			$("#downloadLink").attr('href','data:text/csv;charset=utf-8,' + escape(jsonData));
			$("#downloadLink").attr('download', fileName);
