...
app.get("/visualizeDevices", function(req, res){
  var devices = {};
  db.open(function (err, db) {
    if (!err) {
      console.log("Connecting to tweets database");
...
app.get("/visualizeLanguages", function(req, res){
  var languages = {};
  db.open(function (err, db) {
    if (!err) {
      console.log("Connecting to tweets database");
...
app.get("/visualizeTimeZones", function(req, res){
  var timeZones = {};
  db.open(function (err, db) {
    if (!err) {
      console.log("Connecting to tweets database");
      db.collection('tweets', { strict: true }, function (err, collection) {
        collection.find({}).toArray(function(err, result){
          if(!err){
            var source = "";
            for(var i=0; i <result.length; i++){
              timeZone = result[i]["user_time_zone"];
